...
def main(train_path, test_path, out_path):
    ...
    # Limit the maximum number of iterations for SVR to cap training time. The
    # kernel and overall SVR method remain unchanged.
    model = Pipeline([
        ("scaler", StandardScaler(with_mean=True, with_std=True)),
        ("svr", SVR(kernel="rbf", C=10.0, epsilon=0.1, gamma="scale", max_iter=5000))
    ])
